...
#[cfg(feature = "tokio")]
impl<R: AsyncRead + BufRead + Unpin> AsyncRead for DeflateEncoder<R> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context,
        buf: &mut [u8],
...
#[cfg(feature = "tokio")]
impl<R: AsyncWrite + BufRead + Unpin> AsyncWrite for DeflateEncoder<R> {
...
#[cfg(feature = "tokio")]
impl<R: AsyncRead + BufRead + Unpin> AsyncRead for DeflateDecoder<R> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context,
        buf: &mut [u8],
...
#[cfg(feature = "tokio")]
impl<R: AsyncWrite + BufRead + Unpin> AsyncWrite for DeflateDecoder<R> {
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context, buf: &[u8]) -> Poll<io::Result<usize>> {
        AsyncWrite::poll_write(Pin::new(self.get_mut().get_mut()), cx, buf)
    }
