...
///
/// This structure implements a [`AsyncWrite`] interface and takes a stream of
/// uncompressed data, writing the compressed data to the wrapped writer.
///
/// [`AsyncWrite`]: https://doc.rust-lang.org/std/io/trait.AsyncWrite.html
///
/// # Examples
///
/// ```
...
///
/// // Vec<u8> implements AsyncWrite to print the compressed bytes of sample string
/// # fn main() {
///
...
impl<W: AsyncWrite> DeflateEncoder<W> {
    /// Creates a new encoder which will AsyncWrite compressed data to the stream
    ...
    /// calls to `AsyncWrite` may result in a panic.
    ///
    /// # Panics
    ///
    /// Attempts to AsyncWrite data to this stream may result in a panic after this
    /// function is called.
    ///
    /// # Errors
    ///
...
impl<W: Read + AsyncWrite> Read for DeflateEncoder<W> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.inner.get_mut().read(buf)
    }
}

impl<W: AsyncRead + AsyncWrite + Unpin> AsyncRead for DeflateEncoder<W> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context,
        buf: &mut [u8],
...
///
/// This structure implements a [`AsyncWrite`] and will emit a stream of decompressed
/// data when fed a stream of compressed data.
///
/// [`AsyncWrite`]: https://doc.rust-lang.org/std/io/trait.Read.html
///
/// # Examples
///
/// ```
...
/// use std::io;
/// # use flate2::Compression;
/// # use flate2::AsyncWrite::DeflateEncoder;
/// use flate2::AsyncWrite::DeflateDecoder;
///
/// # fn main() {
/// #    let mut e = DeflateEncoder::new(Vec::new(), Compression::default());
/// #    e.write_all(b"Hello World").unwrap();
...
/// // Here Vec<u8> implements AsyncWrite
...
impl<W: AsyncWrite> DeflateDecoder<W> {
    /// Creates a new decoder which will AsyncWrite uncompressed data to the stream.
    ///
    /// When this encoder is dropped or unwrapped the final pieces of data will
    /// be flushed.
...
impl<W: AsyncRead + AsyncWrite + Unpin> AsyncRead for DeflateDecoder<W> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context,
        buf: &mut [u8],
